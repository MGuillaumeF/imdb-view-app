...
export default function ToggleButtonGroup({buttons, onChangeToggle, defaultValue, ...props}: IToggleButtonGroupProps): ReactElement {
    const [activeButton, setActiveButton] = useState<string>(defaultValue || '');
    return (
        <div className='ToggleButtonGroup' {...props}>
            {
                buttons.map(({value, objectContent, onClick, ...buttonProps} : IToggleButtonProps) => {
                    return <Button {...buttonProps} key={buttonProps.id} onClick={(e) => {
                        if (onClick) {
                            onClick(e);
                        }
                    ...
                    className={`${buttonProps.className || 'Button'} ${activeButton === value ? 'active' : ''}`} >{objectContent || value}</Button>
                })
            }
        </div>
    )
